      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((n) => (
          <Link
            key={n.id}
            href={`/brief/${n.id}`}
            className="flex gap-3 rounded-xl border border-border bg-card p-4 transition hover:border-red-600/35 hover:bg-muted/40"
          >
            <span className="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-red-600" aria-hidden />
            <div className="min-w-0">
              <p className="text-xs font-semibold text-red-600">{n.category}</p>
